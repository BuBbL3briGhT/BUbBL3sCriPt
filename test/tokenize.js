...
describe("tokenize(string)", function() {
  ...
  // Helper `Vektar.blow` from Lista can be used to construct expected lists.
  // Or `Lista.from` for arrays.
...
// expectedTokenObjectsList is a Lista of token objects {type, value, line, column}
function itTokenizes(s, ...expectedTokens) {
  // If expected is just one item and not a lista, wrap it for consistency if Vektar.blow doesn't handle single items.
  // Lista.blow should handle if it's a single object by creating a lista of one.
  it(`tokenizes "${s}"`, function() {
    const actualTokens = tokenize(s);
    // For deep equality on Lista, we might need to convert both to arrays.
    // Or ensure Lista has a custom equality check recognized by assert.deepEqual.
    // For now, let's convert to arrays if Lista is complex.
    // However, the problem description implies Lista can be used directly with deepEqual
    // if its structure and elements are simple objects.
    // Let's try direct comparison first.
    // The `tokenize` function already returns an inverted (natural order) lista.
