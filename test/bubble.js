...
describe("Bubble", () => {
  ...
  describe("#conj(bubble)", function () {
    it("conjoins bubble", function () {
       const bubble = Bubble.make(1, 2, 3);
      const list2 = Bubble.make(4, 5, 6);

      assert.deepEqual(
        Bubble.make(1, 2, 3, 4, 5, 6),
        list2.conj(bubble));
    });
  });
  ...
  describe("#zip(bubble)", function () {
    ...
    it("zips uneven lists", function () {
      const list1 = Bubble.make(1,2,3);
      const list2 = Bubble.make(4,5,6,7,8);
      const result = list1.zip(list2);
      assert.deepEqual([...result],
        [1,4,2,5,3,6,7,8]);
  ...
  describe("#zip(bubble)", function () {
    it("unzips a bubble", function () {
      const bubble = Bubble.make(1,4,2,5,3,6);
    ...
    it("unzips uneven bubble", function () {
      const bubble = Bubble.make(1,5,2,6,3,7,4);
      const result = bubble.unzip();
      assert.deepEqual(result,
        Bubble.make(
          Bubble.make(1,2,3,4),
  ...
  describe("make(o...)", () => {
    it("makes a bubble", () => {
      assert.equal(Bubble.make(), Bubble.emptyList);
      let bubble = Bubble.make(1, 2, 3);
  ...
  describe("find(value)", function () {
    it("returns first instance of value if found in bubble, otherwise undefined", function () {
      let bubble = Bubble.make(1, 2, 3);
      assert.deepEqual(bubble.find(1), Bubble.make(1, 2, 3));
      assert.deepEqual(bubble.find(2), Bubble.make(2, 3));
      assert.deepEqual(bubble.find(3), Bubble.make(3));
  ...
  describe("until(value)", function () {
    it("returns a copy of the bubble truncated to the item immediatly before the first instance of value", function () {
      let bubble = Bubble.make(1, 2, 3);
      assert.deepEqual(bubble.until(2), Bubble.make(1));
      assert.deepEqual(bubble.until(3), Bubble.make(1, 2));
  ...
  describe("split", function () {
    ...
    it("should split a bubble", function () {
      ...
      bubble = Bubble.make(1, 2, sAmp, 3);
      assert.equal(bubble.split(Ṣymbol.for("&")).toString(),
        "((1 2) (3))");

      bubble = Bubble.make(1, sAmp, 2, sAmp, 3);
      assert.equal(bubble.split(Ṣymbol.for("&")).toString(),
        "((1) (2) (3))");

      bubble = Bubble.make(1, 2, 3, sAmp);
      assert.equal(bubble.split(Ṣymbol.for("&")).toString(),
        "((1 2 3) ())");

      bubble = Bubble.make(sAmp, 1, 2, 3);
      assert.equal(bubble.split(Ṣymbol.for("&")).toString(),
        "(() (1 2 3))");

      bubble = Bubble.make(1, sAmp, sAmp, 2, 3);
      assert.equal(bubble.split(Ṣymbol.for("&")).toString(),
        "((1) () (2 3))");

      bubble = Bubble.make(1, sAmp, sAmp, sAmp, 2, 3);
      assert.equal(bubble.split(Ṣymbol.for("&")).toString(),
        "((1) () () (2 3))");
    });
  ...
  describe("Symbol.iterator", () => {
    ...
    it("should yield the single element for a single-element bubble", () => {
      const bubble = Bubble.make(1); // Creates bubble: 1 -> emptyList
      const expected = [1];
    ...
    it("should correctly yield null and undefined values if they are part of the bubble", () => {
      const bubble = Bubble.make(1, undefined, 3, null, 5); // Expected order: 5, null, 3, undefined, 1
    ...
    it("should not modify the original bubble during iteration", () => {
      const bubble = Bubble.make("a", "b", "c"); // c -> b -> a
    ...
    it("should allow multiple iterations independently", () => {
      const bubble = Bubble.make("x", "y"); // y -> x
      const expected = ["x", "y"];
  ...
  describe("partition", function () {
     it("partitions a bubble", function () {
       let bubble = Bubble.make(1, 2, 3, 4, 5);
  ...
  //     let bubble = Bubble.make("hello", "hola");
