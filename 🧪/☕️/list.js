...
describe('List', () => {
  describe('#conj(list)', function () {
    it('conjoins list', function () {
      ...
      assert.deepEqual(List.make(1, 2, 3, 4, 5, 6), list2.conj(list));
    });
  });
  ...
  describe('#zip(list)', function () {
    ...
    it('zips uneven lists', function () {
      ...
      const result = list1.zip(list2);
      assert.deepEqual([...result], [1, 4, 2, 5, 3, 6, 7, 8]);
      const result2 = list2.zip(list1);
      assert.deepEqual([...result2], [4, 1, 5, 2, 6, 3, 7, 8]);
    });
  });
  ...
  describe('#zip(list)', function () {
    it('unzips a list', function () {
      ...
      assert.deepEqual(
        result,
        List.make(List.make(1, 2, 3), List.make(4, 5, 6)),
      );
    });
    it('unzips uneven list', function () {
      ...
      assert.deepEqual(
        result,
        List.make(List.make(1, 2, 3, 4), List.make(5, 6, 7)),
      );
    });
  });
  ...
  describe('#reduce(fn)', () => {
    it('reduces the list', () => {
      ...
      ((list = list.push(3)), (result = list.reduce(add)));
  ...
  describe('#skip(count)', () => {
    it('skips', () => {
      ...
      assert.equal(list.skip().toString(), '(6 7 8)');
  ...
  describe('split', function () {
    it('should split a list', function () {
      ...
      let list = List.make(1, 2, 3);
      assert.equal(list.split(Ṣymbol.for('&')).toString(), '((1 2 3))');
  ...
  describe('Symbol.iterator', () => {
    it('should not yield any values for an empty linkedList (LinkedList.air)', () => {
      ...
      assert.equal(
        count,
        0,
        'for...of loop should not execute for List.emptyList',
      );
    });
    ...
    it('should handle a list created by new List() (yields initial undefined value)', () => {
      ...
      assert.deepEqual(
        results,
        [undefined],
        "Default constructor List should yield its undefined 'o' value",
      );
    });
    ...
    it('should yield the single element for a single-element list', () => {
      ...
      assert.deepEqual(
        resultsSpread,
        expected,
        'Spread syntax results mismatch',
      );
    });
    ...
    it('should yield all elements in a multi-element list in order', () => {
      ...
      assert.deepEqual(
        resultsForOf,
        expected,
        'for...of loop results mismatch for multi-element list',
      );
      ...
      assert.deepEqual(
        resultsSpread,
        expected,
        'Spread syntax results mismatch for multi-element list',
      );
    });
    ...
    it('should correctly yield null and undefined values if they are part of the list', () => {
      ...
      assert.deepEqual(
        results,
        expected,
        'Spread syntax results mismatch for list with null/undefined',
      );
    });
    ...
    it('should not modify the original list during iteration', () => {
      ...
      assert.deepEqual(
        [...list],
        expectedItems,
        'First iteration results mismatch',
      );
      ...
      assert.equal(
        list.toString(),
        expectedToString,
        'List string form changed after iteration',
      );
      ...
      assert.deepEqual(
        [...list],
        expectedItems,
        'Second iteration results mismatch',
      );
    });
