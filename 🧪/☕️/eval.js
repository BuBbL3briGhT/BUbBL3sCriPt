...
import { ėval, evalEach, evalExpression } from '../../☕️/eval.js';
...
describe('ėval', function () {
  ...
  it('expands a more complex macro', function () {
    ...
    let fn = evalEach(bnd, parse('((fn 🪻) (* 6 9) ' + '(🐒 1 2 🪻) (+ 3 4))'));
  ...
  describe('function parameters', function () {
    it('splats', function () {
      const bnd = Object.create(rootBinding);
      assert.equal(
        ėval(bnd, '(((fn a b c) (list b c a)) 1 2 3)').toString(),
        '(2 3 1)',
      );
      assert.equal(
        ėval(bnd, '(((fn a & b) (list a b)) 1 2 3)').toString(),
        '(1 (2 3))',
      );
      assert.equal(
        ėval(bnd, '(((fn & a) (send a :pop)) 1 2 3)').toString(),
        '(2 3)',
      );
      // assert.equal(ėval(
      //   "((fn [& a] (pop a)) 1 2 3)")
    ...
    it('destructures', function () {
      const bnd = Object.create(rootBinding);
      assert.equal(
        ėval(bnd, '(((fn a b) (list a b b)) 1 [2 3])').toString(),
        '(1 [2 3] [2 3])',
      );
      assert.equal(
        ėval(bnd, '(((fn a [b c]) (list a b c)) 1 [2 3])').toString(),
        '(1 2 3)',
      );
    });
